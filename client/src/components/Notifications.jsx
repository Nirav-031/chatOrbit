...
function Notifications({ isOpen, onClose }) {
  ...
  const createChat = (senderId) => {
    ...
    try {
      setLoading(true);
      axios.defaults.withCredentials = true;
      axios.put("http://localhost:3000/acceptRequest", { senderID: senderId, receiverID: userID.user.id })
        .then((result) => {
          setLoading(true);
          // console.log(result);
  ...
  const deleteRequest = (senderId) => {
    try {
      ...
      axios.defaults.withCredentials = true;
      axios.put("http://localhost:3000/declineRequest", { senderID: senderId, receiverID: userID.user.id })
        .then((result) => {
          setLoading(true);
